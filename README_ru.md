...
Локальная директория является источником истины: после успешного применения правило будет содержать ровно тот набор 
паттернов, который находится в указанной директории.

Команды `plan` и `apply` не создают правила и не изменяют их метаданные в appScreener. Команда `init-rule` создаёт 
только локальную заготовку проекта и не обращается к API appScreener.
...
- создание локальной заготовки нового правила командой `init-rule`.
...
## Создание проекта правила

Команда `init-rule` создаёт локальную структуру директорий для разработки нового правила appScreener. Для неё не нужен 
JWT, она не обращается к сети и не создаёт правило в appScreener.
...
Регистр значения типа паттерна не учитывается. Допустимы:
...
Созданный `patterns.yaml` содержит выбранный тип паттернов, уровень критичности, уверенность и состояние активности:
...
Имя директории не обязано соответствовать формату `cwe<number>`. Можно использовать любое допустимое имя из одного 
компонента пути.

Команда:

- требует, чтобы корневая директория правил уже существовала;
- не перезаписывает существующую целевую директорию;
- блокирует обход пути и зарезервированные Windows-имена;
- сначала создаёт проект во временной директории;
- атомарно переносит готовый проект в конечную директорию;
- создаёт `.gitkeep` в пустых директориях.
...
- команды `plan` и `apply` работают только с существующим кастомным правилом;
- команда `init-rule` создаёт только локальную структуру проекта, но не правило на сервере appScreener;
- изменение метаданных правила на сервере не поддерживается;
